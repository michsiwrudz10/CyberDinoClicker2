...
export default function PurchaseProposalModal({
  open = false,
  product = null,
  badge = "Premium Offer",
  title = "Unlock premium content",
  description = "Buy this premium item from the server-backed checkout flow.",
  busy = false,
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: 14 }}>
          <div style={{ padding: 18, borderRadius: 20, background: "linear-gradient(180deg, rgba(56,189,248,0.14), rgba(20,184,166,0.08))", border: "1px solid rgba(56,189,248,0.18)" }}>
            <div style={{ fontSize: 14, color: "#67e8f9", fontWeight: 800, letterSpacing: 0.3, textTransform: "uppercase" }}>{t("purchase.instantBonus", {}, "Instant bonus")}</div>
            <div style={{ marginTop: 10, fontSize: 40, fontWeight: 900 }}>{rewardLabel}</div>
            <div style={{ marginTop: 12, color: "#cbd5e1", lineHeight: 1.5 }}>{t("purchase.serverGranted", {}, "After payment confirmation the bonus is granted from the server. You can keep browsing while the checkout opens.")}</div>
          </div>
